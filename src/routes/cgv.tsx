...
export const Route = createFileRoute("/cgv")({
  loader: async ({ context }) => {
    const page = await context.queryClient.ensureQueryData(editorialPageQuery("cgv"));
  ...
  head: ({ loaderData }) => ({
    ...
    links: [{ rel: "canonical", href: "https://hbs-home.com/cgv" }],
  }),
...
function CgvPage() {
  const { data: page } = useSuspenseQuery(editorialPageQuery("cgv"));
